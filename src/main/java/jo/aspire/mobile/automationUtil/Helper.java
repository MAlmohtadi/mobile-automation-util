...
//		}
	...
	public MobileElement wait(By locator, int second) {
		WebDriverWait driverWait = new WebDriverWait(driverProvider.getCurrentDriver(), second);
		return w(driverWait.until(ExpectedConditions.visibilityOfElementLocated(locator)));
	}
	...
	public BufferedImage takeShot() {
		for (int i = 0; i < 5; i++) {
			if (currentDriver().manage().window().getSize().getHeight() > currentDriver().manage().window().getSize()
				...
				if (image != null) {
					// System.out.println(" original image height:
					...
					// System.out.println(" rotated image height:
					// "+image.getHeight()+", width: "+image.getWidth());

					image = image.getSubimage(diff, diff, image.getWidth() - diff, image.getHeight() - diff);
					// System.out.println(" result image height:
					// "+image.getHeight()+", width: "+image.getWidth());
					return image;
				}
			}
